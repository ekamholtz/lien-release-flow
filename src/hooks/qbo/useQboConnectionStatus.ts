...
export function useQboConnectionStatus(companyId?: string) {
  ...
  // Use Record<string, any> instead of Record<string, unknown>
  ...
  const checkQboConnection = async (currentCompanyId: string) => {
    try {
      ...
      // Get QBO connections for this company
      ...
      if (error) {
        ...
        throw new Error(`Failed to check QBO connection: ${error.message}`);
      }
