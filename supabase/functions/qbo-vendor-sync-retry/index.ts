...
serve(async (req) => {
  ...
  try {
    ...
    // Get failed vendor syncs with retry attempts < 5
    ...
    if (!failedSyncs?.length) {
      return new Response(
        JSON.stringify({ message: 'No failed vendor syncs to retry' }),
        ...
        try {
          ...
          const response = await fetch(
            'https://oknofqytitpxmlprvekn.functions.supabase.co/sync-vendor',
            {
              ...
              body: JSON.stringify({ vendor_id: sync.entity_id })
            }
          );
    ...
    await logQboAction(supabase, {
      function_name: 'qbo-vendor-sync-retry',
      payload: { 
    ...
    console.error('Error in vendor-sync-retry:', error);
    return new Response(
