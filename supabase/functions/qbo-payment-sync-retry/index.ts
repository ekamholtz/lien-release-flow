...
serve(async (req) => {
  ...
  try {
    ...
    // Get failed payment syncs with retry attempts < 5
    ...
    if (!failedSyncs?.length) {
      return new Response(
        JSON.stringify({ message: 'No failed payment syncs to retry' }),
      ...
      failedSyncs.map(async (sync) => {
        ...
        try {
          ...
          const response = await fetch(
            'https://oknofqytitpxmlprvekn.functions.supabase.co/sync-payment',
            {
              method: 'POST',
              headers: {
              ...
              body: JSON.stringify({ payment_id: sync.entity_id })
            }
          );
    ...
    await logQboAction(supabase, {
      function_name: 'qbo-payment-sync-retry',
      payload: { 
    ...
    console.error('Error in payment-sync-retry:', error);
    return new Response(
